...
#Create tabs
tab1, tab2, tab3 = st.tabs(["List instances", "Instance properties", "Export instances"])

with tab1:
    
    #Create object instances & Get all instances
    instances = AwsEc2(aws_end_end_point)
    ec2_instances = instances.get_instances()

    #Crete DataFrame
    df = pd.DataFrame(ec2_instances)
...
with tab2:
    ...
    instance_id = st.text_input('Type instance ID:',key ="1")
    data = instances.get_ec2_yml_properties(instance_id)
